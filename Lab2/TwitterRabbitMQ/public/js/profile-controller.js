twitter.controller('profileController', function($scope, $http, $routeParams) {
	...
	var loadProfile = function() {
		var profileResponse = $http.get('/getprofile/' + $routeParams.puid);
		profileResponse.success(function(data) {
			if (data.status == 200) {
				$scope.userprofile = data.profile[0];
				console.log(data);
				console.log("other: " + data.profile[0]);
				if($routeParams.puid == $scope.profile.puid)
	...
	var loadTweets = function() {
		var getTweetsResponse = $http.get("/tweets/" + $routeParams.puid);
	...
	var getNewProfile = function() {
		...
		newProfileResponse.success(function(data) {
			if(data.status === 200) {
				$scope.newguys.concat(data.profile);
			}
		});
	};
